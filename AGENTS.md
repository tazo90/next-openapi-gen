...
- `packages/next-openapi-gen` CLI bundling in `tsup.config.ts` must externalize non-workspace runtime dependencies; bundling CommonJS packages like `commander` into the ESM CLI breaks sample-app `generate` with Node's `Dynamic require` error.
- GitHub Actions CI is defined in `.github/workflows/ci.yml` (quality, build, unit, integration, coverage, and a parallelized E2E matrix on pull requests plus default-branch pushes and `workflow_dispatch`); action dependency bumps are handled via `.github/dependabot.yml`.
- Generator performance benchmarking uses `vitest.bench.config.ts`; `pnpm test:bench` runs the stable profiling entrypoint and `pnpm test:bench:vitest` runs raw Vitest benches under `tests/bench`.
