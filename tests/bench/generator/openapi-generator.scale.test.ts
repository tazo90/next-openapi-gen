...
describe.sequential("OpenApiGenerator scale profiling", () => {
  const scenarios = getBenchmarkScenarios("scale");
  ...
  it("prints stable timing summaries for at-scale fixtures", () => {
    const summaries = scenarios.flatMap((scenario) => {
      const project = projects.get(scenario.id)!;
      return [
        summarizeProfiles(project, "cold", collectProfiles(project, ITERATIONS)),
        summarizeProfiles(project, "warm", collectWarmProfiles(project, ITERATIONS)),
      ];
    });
