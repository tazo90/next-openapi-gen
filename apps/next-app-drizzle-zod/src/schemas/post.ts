...
export const CreatePostSchema = createInsertSchema(posts, {
  ...
  content: (schema) =>
    schema.min(10, "Content must be at least 10 characters").describe("Post content in markdown"),

  published: (schema) => schema.optional().describe("Whether the post is published"),

  authorId: (schema) =>
    schema.positive("Author ID must be positive").describe("ID of the post author"),
});
