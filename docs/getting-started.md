...
`init` writes `openapi-gen.config.ts` by default. Config discovery also accepts:

- `openapi-gen.config.ts` / `.mts` / `.js` / `.mjs`
- `openapi-gen.config.json`
- legacy `next-openapi.config.*`
- legacy `next.openapi.json`

Prefer the modern `openapi-gen.config.*` names. Pass `--output next.openapi.json`
(or any `.json` path) when you want a JSON config. Legacy names still load, but
they emit deprecation warnings.

`init` writes a `defineConfig` module. A minimal typed config looks like:
